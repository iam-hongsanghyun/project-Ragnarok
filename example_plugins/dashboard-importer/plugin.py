...
def analyze(result: dict[str, Any] | None, config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not rows:
        return {
            "note": "No generators found in the model — pick a model workbook above."
        }
...
def fillReallocation(config: dict[str, Any]) -> dict[str, Any]:  # noqa: N802 — manifest hook name
    ...
    year — or the whole fleet when ``replace_include_existing`` is on — and
    ...
    if not plan:
        return {
            ...
            "message": (
                ...
                f"year — retired units included — and on/after the replacement base "
    ...
    return {
        ...
        "message": (
            ...
            f"frozen at the current settings — change settings and fill again to "
