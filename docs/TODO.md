...
| `X4` | `Medium` | `Both` | **Rename a stored History run.** A run is named by UTC timestamp + a derived label and can't be renamed today. Add an inline rename on the History row (and surface the editable name in the run viewer) + a guarded backend rename endpoint that renames the `<name>.json` / `.meta.json` / `.xlsx` triplet together, updates the `name` inside the meta, rejects collisions and unsafe names, and clears `activeRunName` if it pointed at the old name. The label shown in History/Comparison should follow the new name. | History is now the single source of truth for past runs, so a human-readable, user-chosen run name is the high-value bit users actually want instead of a raw timestamp. Small, self-contained. | 8,000 |
