...
|---|---|---|
| **1. Data-in for arbitrary regions** (highest leverage) | general weather → renewable profiles (**D1** + **I4**), fuel & carbon prices (**I5**), driver-based demand (**I3**), any-country network (**I9**) | **started** — keyless any-coordinate wind/solar profiles now land via the Open-Meteo importer; caching (**D1**), fuel prices (**I5**), demand forecast (**I3**) remain |
...
| ID | Status | Pri | Surface | Task | Why | Cost |
|---|---|---|---|---|---|---:|
| `D1` | `In progress` | `High` | `Backend` | **Profile / weather data layer** — persistent storage, source registry (versioning + provenance), and source-health checks for renewable, weather, fleet, grid, and policy datasets. **Done:** the source registry + per-database module pattern (`importers/registry.py`, `importers/databases/*`); a live keyless weather source is wired (Open-Meteo ERA5, via `openmeteo_renewable`). **Remaining:** the **caching / provenance layer** (Open-Meteo is fetched live per request — fine for small windows, but full-year × many-point fetches need a cached store; also the redistribution-limited Renewables.ninja needs caching). | Owns caching, versioning, provenance, and health for every external dataset. Now that a weather source is live, this is the scale/robustness layer under it. | 14,000 |
...
| `I4` | `In progress` | `High` | `Both` | **Renewable resource profile importer** — fetch wind / solar / hydro-inflow capacity-factor series for a location and land them as `generators-p_max_pu`. **Done:** per-bus KPG193 profiles (`kpg193_renewable_profile`); **and the general, keyless, any-coordinate importer** — `openmeteo_renewable` (Open-Meteo ERA5 global archive) queries the region's representative point and lands a bus + solar/wind generator(s) + weather-derived `p_max_pu` (region-based / create-for-siting entry point; verified live). **Remaining:** attach-to-*existing*-fleet by each generator's coordinate (a session-model transform, not an importer), polygon / multi-point sampling, extra sources (PVGIS / NASA POWER / BYOK Renewables.ninja), and caching via **D1**. See *I4 design* below. | A single national profile is too coarse for siting. Coordinate-driven attachment + snapshot alignment (via **T1**) puts the right shape on the right asset. | 10,000 |
