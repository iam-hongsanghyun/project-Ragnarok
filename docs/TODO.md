...
| `I3` | `Done` | `High` | `Both` | **Driver-based demand forecast generator** — **Shipped 2026-07-02** (`app/demand_drivers.py`: macro pop/GDP-elasticity factor + winter-peaking heat and overnight EV electrification components with own hourly shapes → the demand SHAPE evolves, peak can move to winter; `POST /api/session/snapshots/driver-forecast` + Forge Temporal panel; 7 analytical tests). *Was:* per-bus / per-region future demand profiles from drivers (population, GDP, electrification rate, weather sensitivity) with hourly reshaping for pathway runs. | Pathway runs need decade-spanning demand with an evolving *shape*. Distinct from **T1** (scales an existing series) — this derives a *new* shape from drivers. The World Bank importer fetches annual population/per-capita drivers but no hourly-shape generator exists. Depends on **D1** for driver datasets. | 24,000 |
| `I7` | `Done` | `Med` | `Frontend` | **Capacity-factor / generation history (Ember / IEA)** — **Shipped 2026-07-02** (`ember_history` database, Ember Data API v1, BYOK `ember_key`, global: month × fuel generation → `generation_history` sheet, fuels mapped to Ragnarok carriers; IEA variant not pursued — Ember covers the calibration need keylessly-cheap). *Was:* country-by-month / -year generation by carrier, for analytics calibration and as a fallback when hourly data is unavailable. | A cheap sanity check that catches order-of-magnitude model errors. Ember monthly data is CC-BY; IEA needs free registration. No Ember/IEA importer exists. | 10,000 |
...
| `D1` | `Done` | `High` | `Backend` | **Profile / weather data layer** — **Completed 2026-07-02:** general multi-source cache (`importers/cache.py`, RAGNAROK_IMPORT_CACHE, TTL/immutable, hard NEVER_CACHE licence guard for renewables_ninja; wired into ember_history), source health probes (`importers/health.py` + GET /api/import/health — 401/403 = up-needs-key), and a persistent versioned provenance log (`importers/provenance_log.py` + GET /api/import/provenance, recorded on every /api/import/run). The Open-Meteo weather cache keeps its own dir for back-compat. *Was:* persistent storage, source registry, source-health checks, versioned provenance. **Done:** source registry + per-database module pattern; keyless Open-Meteo weather; **on-disk weather caching** (`openmeteo_renewable/cache.py`, `RAGNAROK_WEATHER_CACHE`). **Remaining:** source-health checks, versioned provenance, and a **general (multi-source) cache abstraction** beyond the Open-Meteo-specific one. | Owns caching, versioning, provenance, health for every external dataset. Weather caching exists; the general layer + health/provenance remain. | 8,000 |
...
| `L1` | `Open` | `High` | `Both` | **Bifrost — AI conversational model builder** — **Design doc: [`docs/bifrost-agent.md`](bifrost-agent.md)** (tool catalog over the existing API, pluggable LLMProvider incl. Claude API / Claude Code / local Hermes, plan-act-observe loop, verification via Q2/Ember/A2, phased build starting with an MCP prototype). Stand-alone project (React/TS chat shell + thin Python relay) (React/TS chat shell + thin Python relay). The user asks a question; Bifrost runs an **agentic tool-use loop** deciding which Ragnarok importer / schema tools to call to assemble a workbook (same JSON schema `POST /api/run` accepts), then lets the user inspect or run. Data gaps are resolved by asking, not hallucinating. Brain: **Claude Opus 4.8** (or **Sonnet 4.6**) via a thin relay reusing the BYOK key store. Bifrost owns no solver / DB / editor — it delegates to Ragnarok. | LLM answers to energy questions are unverifiable; routing through a real PyPSA solve makes every conclusion falsifiable. Not started. | 36,000 |
...
| ID | Status | Pri | Surface | Task | Why | Cost |
|---|---|---|---|---|---|---:|
| `X1` | `Done` | `Medium` | `Both` | **Backend-centric data processing (thin browser)** — **Shipped 2026-07-02.** The `deriveRunResults` port landed WITHOUT re-porting TS: `run_pypsa`'s post-solve tail was extracted verbatim into `_build_solved_payload` (shared assembly), and `results/derive_outputs.py::derive_results_from_outputs` reconstructs a solved network from a bundle's stored outputs (`_attach_outputs` = exact inverse of `build_full_outputs`) and reruns that same assembly — so imported bundles get analytics identical to a fresh solve. Wired into `import_bundle_from_upload` (`_fill_derived_analytics`, best-effort: pathway/malformed bundles fall back to client derivation, which stays for pathway period-interactivity). Parity gate `test_derive_outputs` pins derived == fresh-solve on 28 fields × 3 model classes. Only **X5** (plugin Worker sandbox, tracked separately) remains in this direction. *History:* session store shipped; **server-side derived chart-series** (`results/derived_series.py` + `GET /api/runs/{name}/derived/{metric}`); **import-KPI + analyser derivations** server-side (**X2**). **Remaining (the hard part):** port the 766-line client `deriveRunResults` (App.tsx import + pathway path) to Python so imported bundles derive analytics server-side. **Risk note:** the backend deliberately does NOT derive on import (`project_workbook.py`) and `run_pypsa` always re-solves, so this is a genuine faithful port where a subtle error corrupts analytics *silently* (a crash-only fallback won't catch it) — needs a Q1-style parity-fixture pass, not a blind rewrite. Plus JS-plugin sandbox (**X5**). *Was:*. **Done:** the stateful server-side session store (`model_store` / `sqlite_store` / `session_store` + `/api/session/*`), server-held time-series with windowed/downsampled fetch, server-side Python plugins, light stored-run views. **Remaining:** porting `deriveRunResults` / chart-series derivation server-side (still in `lib/results/runResults.ts`), JS-plugin sandboxing (**X5**), full push/poll sync. | Large models make a browser-resident app heavy; the session-store direction shipped, the analytics-derivation port remains. | 30,000 |
| `X2` | `Done` | `Medium` | `Both` | **Data-import KPI computation → backend API** — **Shipped 2026-07-02** (`app/analysis.py`: `column_statistics` + the analyser derivations `duration_curve`/`daily_profile`/`grouped_aggregate`, served at `GET /api/session/sheet/{name}/stats` and `/derive`; ColumnStatsPanel + client fns render server-computed results so the browser needn't crunch an 8760-row sheet. *Remaining polish:* wire the InputAnalyser component itself to fetch `/derive` and drop its whole-sheet load — a UI change for an in-app pass.). *Was:* — move the remaining client-side analysis of imported data (`InputAnalyser.tsx` in-browser KPI/statistics) to an endpoint; the frontend just renders. The import *preview* is already backend-computed. A concrete, low-risk slice of **X1**. | Keeps heavy per-row statistics off the main thread and centralises KPI definitions. | 10,000 |
| `X6` | `Done` | `Medium` | `Both` | **Richer / clearer plugin output scheme** — **Shipped 2026-07-02** (contract v2: `multiRun` manifest flag → `analyze(result, config, runs)` with stored-run analytics resolved server-side, so analytics-over-N-runs ship as plugins; reserved `layout` output key arranges outputs into host-rendered rows, validated server-side; `contractVersion` surfaced; documented in docs/plugin.md; 4 new tests, v1 plugins untouched). *Was:* extend the plugin contract: (a) declarative **composite host-rendered layouts** (chart grids with shared legend/settings); (b) **multiple runs / scenarios** as input (analytics-over-N-runs); (c) one crisp versioned contract. Grid `inputLayout`/`outputLayout` exists; multi-run + composite layouts remain. Keep "host owns rendering, no raw HTML/SVG". | The scenario-comparison matrix couldn't be a plugin (single-run + no custom layout). Pairs with **X1** / **X5**. | 20,000 |
| `X3` | `Done` | `Low` | `Both` | **Scenario library vs. run history — DECIDED 2026-07-02:** the scenario library is **kept and repositioned as "named run-configuration presets"** — forward-looking, cheap, model-scoped configuration bundles — explicitly distinct from History, which is the authoritative record of executed runs. No deprecation: presets earn their keep for comparing configurations before running (and scenario round-trip via `RAGNAROK_Scenarios` stays for project exchange). The Scenarios section copy now states the positioning ("presets describe what you plan to run; History records what you ran"). No further build planned. | History captures "what I ran" comprehensively; the library's remaining job is pre-run configuration management, which it does. | 0 |
...
| ID | Status | Pri | Surface | Task | Why | Cost |
|---|---|---|---|---|---|---:|
| `H2′` | `Done` | `Medium` | `Both` | **Pluggable result-mapper registry + raw-sheet surfacing** — **Shipped 2026-07-02** (`app/result_mappers.py` registry consulted before canonical reconstruction, first match wins; backend plugins join via `result_mapper_matches`/`result_mapper_map` hooks; unrecognised sheets no longer pollute the model — kept verbatim on `result.rawSheets` and surfaced by `RawSheetsCard`; 5 tests). *Was:* the **core** of H2 shipped (`POST /api/import/result`). What remains: per-source-format **column-mapping rules in a small registry** (extendable via a `result_mapper` plugin hook) so arbitrary third-party result layouts map onto `outputs.{static,series}`, and **unrecognised sheets stored verbatim + surfaced as raw tables**. | Today's reconstruction handles Ragnarok's own schema + canonical bare workbooks; a true third-party layout needs a mapping layer. Do it when a real third-party format needs it. | 10,000 |
