...
| `I4` | `In progress` | `High` | `Both` | **Renewable resource profile importer** — fetch wind / solar / hydro-inflow capacity-factor series for a location and land them as `generators-p_max_pu`. **Done:** per-bus KPG193 profiles; the general keyless any-coordinate **importer** (`openmeteo_renewable`, Open-Meteo ERA5) with **multi-point grid sampling**; the **attach-to-existing-fleet transform** (`POST /api/transform/renewable-profiles` + Forge "Attach renewable profiles" — resolves each renewable generator's coord [own x/y → bus x/y], fetches once per 0.1° cell, merges the profiles); and **on-disk caching** (see **D1**). All verified live. **Remaining:** extra sources (PVGIS / NASA POWER / BYOK Renewables.ninja) and hydro-inflow. See *I4 design* below. | A single national profile is too coarse for siting. Coordinate-driven attachment + snapshot alignment (via **T1**) puts the right shape on the right asset. | 6,000 |
...
| ID | Status | Pri | Surface | Task | Why | Cost |
|---|---|---|---|---|---|---:|
| `M1` | `In progress` | `High` | `Both` | **Sector coupling** — multi-carrier buses + conversion Links (electrolyser, CCGT, heat pump, boiler, fuel cell, …). **Done:** ingestion of multi-carrier buses / Links / Stores (generic component path), the schema-driven Model/Build editors for them (incl. multi-port Links), **per-carrier energy-balance analytics** (`results/energy_balance.py` + card), correct **conversion emissions** (counted at the fuel generator at primary energy, efficiency-aware via **M3** — verified, no double-count), bus `carrier` settable in the Build form, carrier filtering via the Pivot chart, and a **conversion-technology template library** (`lib/build/conversions.ts` + ConversionPicker on the Build Links step — one-click CCGT/OCGT/CHP/electrolyser/fuel-cell/heat-pump/resistive/gas-boiler/methanation, each wiring the Link + counterpart carrier bus + carriers + fuel supply). **Remaining (polish):** dispatch-mix donut still groups generators by fuel carrier so fuel-supply generators (e.g. gas) appear alongside electricity — the per-carrier energy-balance card is the correct multi-vector view; non-geographic (CO₂/H₂) bus map handling; sector-data importers (later). | The decarbonisation studies users want need >1 vector. Engine + core analytics + authoring shipped; what's left is dispatch-donut carrier awareness. | 4,000 |

#### M1 remainder — what's left after the core shipped

The optimiser needs nothing (PyPSA does multi-carrier LP natively), the schema is complete, the **per-carrier energy balance + emissions** exist, and the **conversion template library** (curated tech defaults + one-click wiring) shipped. Remaining app-layer polish: (1) the **dispatch-mix donut** (`results/dispatch.py::dispatch_by_carrier`) groups ALL generators by fuel carrier, so in a sector-coupled model the fuel-supply generators (gas) show as "generation" mixed with electricity MWh — needs a filter to electricity-bus generators (careful: dispatch.py feeds merchant / company / finance result paths, so update with tests); (2) non-geographic carrier handling on the map (abstract CO₂/H₂ buses with no x/y — template-created buses inherit the anchor's coords, so this only bites hand-made/imported carrier buses); (3) *(deferred)* sector-data importers (gas networks, heat/H₂ demand).
...
- Sources shipped: **OSM** grid topology + **OSM power plants**, **WRI GPPD** fleet, **World Bank** annual demand, **ENTSO-E** installed capacity, **ENTSO-E** hourly load, **EIA-930** hourly demand, **KPG193** Korea pack (network / demand profile / renewable capacity / renewable profile), **Open-Meteo** renewable capacity factors (`openmeteo_renewable` — keyless global ERA5; any-coordinate wind/solar `p_max_pu` via a first-order irradiance/power-curve conversion).
