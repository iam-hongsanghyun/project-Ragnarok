...
function CarbonPriceTab(props: Props) {
  ...
  return (
    <section className="constraints-workspace-section">
      <header className="constraints-workspace-section-header">
        <h3>Carbon price</h3>
        <p>Added to each generator's marginal cost proportional to its carrier's <code>co2_emissions</code> factor.</p>
      </header>
      <div className="sg-setting-row">
        <label className="sg-setting-label" htmlFor="rs-carbon-price">Price (current = applied to all snapshots)</label>
        ...
        <p className="sg-setting-hint">
          Per-year / per-period carbon price schedule will land here in a follow-up PR (currently a single scalar applied to every snapshot).
        </p>
      </div>
    </section>
  );
